...
public class CanaryUtils {
     ...
     */
    public static CanaryResource computeWeightedRoundRobin(final Service service, final ServiceRequest request, final long counter) {
        if (service != null) {
            ...
            if (CollectionUtils.isNotEmpty(template.getResources())) {
                ...
                long baseMod = 0;
                final NavigableMap<Long, CanaryResource> resourceMap = new TreeMap<>();
                ...
                Map.Entry<Long, CanaryResource> resourceEntry = resourceMap.ceilingEntry(counter % baseMod);
